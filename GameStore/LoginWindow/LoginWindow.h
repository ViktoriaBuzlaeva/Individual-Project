...
public ref class LoginWindow : public System::Windows::Forms::Form {
 public:
    GameStore* game_store = new GameStore;
    LoginWindow(void) {
        InitializeComponent();
    }
...
private: System::Void SignInButton_Click
    ...
    if (this->LoginBox->Text != "" && this->PasswordBox->Text != "") {
        std::string login = marshal_as<std::string>(LoginBox->Text);
        std::string password = marshal_as<std::string>(PasswordBox->Text);

        User signing_user(login, password);

        if ((*game_store).check_user_for_signing(signing_user)) {
            MessageBox::Show("You have successfully logged in!");
        } else {
            MessageBox::Show("Login or password incorrect!", "Warning",
                MessageBoxButtons::OK, MessageBoxIcon::Information);
        }
    } else {
        MessageBox::Show("Fill in all boxes!", "Warning",
            MessageBoxButtons::OK, MessageBoxIcon::Information);
    }
}
};
